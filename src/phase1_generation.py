...
from prompts import load_answer_templates, load_prompt_templates
from schema import GenerationResult, QAPair, SharedQuestion

_QA_FIELDS = frozenset(QAPair.model_fields)
# Fixed path outside output/ so shared questions survive --overwrite and output cleanup.
SHARED_QUESTIONS_PATH = Path(__file__).parent.parent / "data" / "shared_questions.json"
...
class DIYDatasetGenerator:
    ...
    def generate_batch(
        ...
        for i, idx in enumerate(schedule):
            ...
            print(
                f"  [{i + 1}/{total}] Generating {template['category']}...",
                end=" ",
                flush=True,
            )
...
def load_shared_questions() -> list[SharedQuestion]:
    """Load the shared question set generated by run_question_generation_phase()."""
    if not SHARED_QUESTIONS_PATH.exists():
        raise FileNotFoundError(f"Shared questions not found: {SHARED_QUESTIONS_PATH}\nRun 'python main.py questions' first.")
    return [SharedQuestion(**r) for r in json.loads(SHARED_QUESTIONS_PATH.read_text())]
...
def run_question_generation_phase(
    ...
    prompt strategy. Saves to data/shared_questions.json at project root.
    """
    import random

    random.seed(seed)

    out_file = SHARED_QUESTIONS_PATH
    ...
    for template in templates:
        ...
        for _ in range(num_per_category):
            ...
            print(
                f"  [{i}/{total}] Generating question for {category}... ",
                end="",
                flush=True,
            )
            try:
    ...
    print(f"\nSaved {len(results)} shared questions → {out_file.resolve()}")
    return results
...
def run_answer_generation_phase(
    ...
    for i, shared_q in enumerate(pending):
        print(
            f"  [{i + 1}/{len(pending)}] Answering {shared_q.category} ({shared_q.trace_id[:8]})... ",
            end="",
            flush=True,
        )
...
def run_generation_phase(
    ...
    if generation_model == "mock":
        generator: Any = BenchmarkGenerator(
            batch_id=batch_id,
            batch_label=batch_label,
            seed=seed,
        ...
        if existing_dicts and not overwrite:
            ...
            remaining_per_category = {
                t["category"]: max(0, target_per_cat - existing_counts.get(t["category"], 0)) for t in generator.templates
            }
