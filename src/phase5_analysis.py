"""
Phase 5: Failure & Quality Analysis
...
FAILURE_MODE_NAMES = [m.name for m in load_failure_modes()]
...
class FailureAnalyzer:
    ...
    def plot_failure_heatmap(self, save_path: Path) -> None:
        ...
        ax.set_yticklabels([m.replace("_", " ").title() for m in mode_cols], rotation=0)
        plt.tight_layout()
    ...
    def plot_failure_rates_by_category(self, save_path: Path) -> None:
        ...
        ax.set_xticklabels([c.replace("_", " ").title() for c in cat_rates.index], rotation=30, ha="right")
        ax.legend(
            [m.replace("_", " ").title() for m in FAILURE_MODE_NAMES],
    ...
    # ------------------------------------------------------------------
    # Visualization 3: before vs after trend (populated by Phase 6)
    # ------------------------------------------------------------------
    def plot_failure_mode_trends(
        ...
        labels = [m.replace("_", " ").title() for m in FAILURE_MODE_NAMES]
    ...
    # ------------------------------------------------------------------
    # Visualization 5: benchmark comparison (placeholder / updated by Phase 7)
    # ------------------------------------------------------------------
    def plot_benchmark_comparison(
        self,
        save_path: Path,
        benchmark_rates: dict[str, float] | None = None,
    ) -> None:
        generated_rates = self.qdf[QUALITY_DIM_NAMES].mean()
        labels = [n.replace("_", " ").title() for n in QUALITY_DIM_NAMES]

        fig, ax = plt.subplots(figsize=(10, 5))
        x = range(len(QUALITY_DIM_NAMES))
        ax.bar([i - 0.2 for i in x], generated_rates, width=0.4, label="Generated", color="#3498db")
        if benchmark_rates:
            bench_vals = [benchmark_rates.get(n, 0) for n in QUALITY_DIM_NAMES]
    ...
    def plot_failure_correlations(self, save_path: Path) -> None:
        ...
        ax.set_xticklabels([m.replace("_", " ").title() for m in FAILURE_MODE_NAMES], rotation=30, ha="right")
        ax.set_yticklabels([m.replace("_", " ").title() for m in FAILURE_MODE_NAMES], rotation=0)
        plt.tight_layout()
        plt.savefig(save_path, dpi=150)
        plt.close()
        print(f"  Saved → {save_path}")
    ...
    # ------------------------------------------------------------------
    def build_summary(self) -> AnalysisSummary:
        ...
        quality_rates = {d: float(self.qdf[d].mean()) for d in QUALITY_DIM_NAMES if d in self.qdf.columns}
        thresholds_met = {
            d.name: quality_rates.get(d.name, 0) >= d.threshold
        ...
        return AnalysisSummary(
            ...
            overall_quality_pass_rate=float(self.qdf["overall_quality_pass"].mean()),
...
def run_analysis_phase(
    ...
    if not failure_csv.exists():
        raise FileNotFoundError(f"Not found: {failure_csv}. Run Phase 3 first.")
    if not quality_csv.exists():
        raise FileNotFoundError(f"Not found: {quality_csv}. Run Phase 4 first.")
    ...
    analyzer.plot_benchmark_comparison(output_dir / "benchmark_comparison.png", benchmark_rates=benchmark_rates)
